...
function createGridMessage(prefix, rows, columns) {
  ...
  for (let y = 0; y < rows; y++) {
    const currRow = Array.from({ length: columns }, (_, x) => `:${prefix}_${y}_${x}:`)
    msgRows.push(currRow.join(''))
  }
  return strToU8(msgRows.join('\n'))
}

async function splitStaticImage(data: ISplitTransferData) {
  return new Promise<Zip>((res) => {
...
self.addEventListener('message', async function ({ data }: { data: ISplitTransferData }) {
  const { filePrefix, columns, rows } = data.state.splitSettings

  const startTime = Date.now()
  console.time('split')

  const zip = data.state.isAnimated ? splitAnimatedImage(data) : await splitStaticImage(data)
